...
const sezioni = [
  ...
  { id:'menu_panini_terra', label:'Panini Terra', icon:'🥪' },
  { id:'menu_panini_mare', label:'Panini di Mare', icon:'🐟' },
  { id:'menu_contorni', label:'Contorni', icon:'🥗' },
    ...
    <div style={{ marginBottom:'16px' }}>
      ...
      {value && (
        <div style={{ position:'relative', marginBottom:'8px' }}>
          <img src={value} alt="" style={{ width:'100%', maxHeight:'180px', objectFit:'contain', borderRadius:'8px', background:'#f3f4f6', display:'block' }} />
  ...
  return (
    <div style={mxw}>
      <div style={crd}>
        ...
        {[['nome','Nome ristorante'],['telefono','Telefono'],['email','Email'],['indirizzo','Indirizzo'],['facebook','Facebook URL'],['instagram','Instagram URL'],['thefork','TheFork URL'],['tripadvisor','TripAdvisor URL'],['maps_url','Google Maps URL']].map(([k,l]) => (
...
function HeroForm({ dati, update }: any) {
  const pagine = [
    { k:'home', l:'Home — Foto hero principale' },
    { k:'menu', l:'Menu — Foto header' },
    { k:'birre', l:'Birre — Foto header' },
    { k:'drink_list', l:'Drink List — Foto header' },
    { k:'vini', l:'Carta dei Vini — Foto header' },
    { k:'galleria', l:'Galleria — Foto header' },
    { k:'contatti', l:'Contatti — Foto header' },
    { k:'allergeni', l:'Allergeni — Foto header' },
  ];
  return (
    <div style={mxw}>
      <div style={{ background:'#f0f4e8', border:'1px solid #6B7F4A', borderRadius:'10px', padding:'12px 16px', marginBottom:'20px' }}>
        <p style={{ color:'#6B7F4A', fontSize:'12px', margin:0 }}>📸 Carica le foto che appaiono in cima a ogni pagina del sito</p>
      </div>
      {pagine.map(p => (
        <div key={p.k} style={crd}>
          <p style={stl}>{p.l}</p>
          <ImageUpload label="Foto" value={dati?.[p.k]||''} onChange={v=>update(p.k,v)} />
        </div>
      ))}
    </div>
  );
      ...
      {(dati?.items||[]).map((item: any, i: number) => (
        <div key={i} style={crd}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'12px' }}>
            <span style={{ color:'#9ca3af', fontSize:'12px' }}>Piatto {i+1}</span>
          ...
          <Field label="Descrizione" value={item.description} onChange={v=>updateItem('items',i,'description',v)} multiline />
          <Field label="Prezzo (es. €12)" value={item.price} onChange={v=>updateItem('items',i,'price',v)} />
          <ImageUpload label="Foto piatto (opzionale)" value={item.foto||''} onChange={v=>updateItem('items',i,'foto',v)} />
        </div>
      ))}
      <button onClick={() => addItem('items',{name:'',description:'',price:'',foto:''})} style={add}>+ Aggiungi piatto</button>
    </div>
  );
}
      ...
      {(dati?.items||[]).map((b: any, i: number) => (
        <div key={i} style={crd}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'12px' }}>
            <span style={{ color:'#9ca3af', fontSize:'12px' }}>Birra {i+1}</span>
            <button onClick={() => removeItem('items',i)} style={rmv}>Rimuovi</button>
          </div>
          ...
          <Field label="Grado (es. 5%)" value={b.abv} onChange={v=>updateItem('items',i,'abv',v)} />
          <Field label="Descrizione" value={b.description} onChange={v=>updateItem('items',i,'description',v)} multiline />
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
            <Field label="Prezzo" value={b.price} onChange={v=>updateItem('items',i,'price',v)} />
            <Field label="Nazione" value={b.origin} onChange={v=>updateItem('items',i,'origin',v)} />
          </div>
        </div>
      ))}
      <button onClick={() => addItem('items',{name:'',brewery:'',style:'',abv:'',description:'',price:'€5',origin:''})} style={add}>+ Aggiungi birra</button>
    </div>
  );
}

function ViniForm({ dati, updateItem, addItem, removeItem }: any) {
  const cats = [['bianchi','I Bianchi'],['rossi','I Rossi'],['rosati','I Rosati'],['calice','Al Calice']];
  return (
    <div style={mxw}>
      {cats.map(([cat,label]) => (
        <div key={cat}>
          ...
          {(dati?.[cat]||[]).map((v: any, i: number) => (
            <div key={i} style={crd}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'12px' }}>
                <span style={{ color:'#9ca3af', fontSize:'12px' }}>{label} {i+1}</span>
              ...
              <Field label="Produttore" value={v.producer} onChange={val=>updateItem(cat,i,'producer',val)} />
              <Field label="Zona/Regione" value={v.region} onChange={val=>updateItem(cat,i,'region',val)} />
              <Field label="Prezzo" value={v.price} onChange={val=>updateItem(cat,i,'price',val)} />
            </div>
          ))}
          <button onClick={() => addItem(cat,{name:'',producer:'',region:'',price:'',description:''})} style={add}>+ Aggiungi vino</button>
        </div>
      ))}
    </div>
  );
...
function GalleriaForm({ dati, updateItem, addItem, removeItem }: any) {
  ...
  return (
    <div style={mxw}>
      <div style={{ background:'#f0f4e8', border:'1px solid #6B7F4A', borderRadius:'10px', padding:'12px 16px', marginBottom:'20px' }}>
        <p style={{ color:'#6B7F4A', fontSize:'12px', margin:0 }}>📷 {items.length} foto totali — carica direttamente dal telefono o PC</p>
      </div>
      ...
      <button onClick={() => addItem('items',{url:'',titolo:''})} style={add}>+ Aggiungi foto</button>
    </div>
  );
}
  ...
  return (
    <div style={mxw}>
      {(dati?.items||[]).map((r: any, i: number) => (
        <div key={i} style={crd}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'12px' }}>
            <span style={{ color:'#9ca3af', fontSize:'12px' }}>Recensione {i+1}</span>
            <button onClick={() => removeItem('items',i)} style={rmv}>Rimuovi</button>
          </div>
          <Field label="Nome cliente" value={r.nome} onChange={v=>updateItem('items',i,'nome',v)} />
          <Field label="Testo" value={r.testo} onChange={v=>updateItem('items',i,'testo',v)} multiline />
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
            <Field label="Stelle (1-5)" value={String(r.stelle||5)} onChange={v=>updateItem('items',i,'stelle',Number(v))} />
